...
class ISearchEngine {
    ...
    createColorDatabaseLS() {
        ...
        //data base com a mesma estrutura do enunciado
        const colorDatabase = { images: [] };

        //inicializa um objeto ColorHistogram
        const h12color = new ColorHistogram(this.redColor, this.greenColor, this.blueColor);
        
        //percorre as imagens do JSON
        for (let i = 0; i < this.jsonData.images.length; i++) {
            const image = this.jsonData.images[i];
            const img = new Picture(0, 0, this.imgWidth, this.imgHeight, image.path, image.category);
            const canvas = this.processingCanvas;
            const context = canvas.getContext('2d');

            // desenhas as imagens do canvas de processamento
            img.imgobj.onload = () => {
                context.drawImage(img.imgobj, 0, 0, this.imgWidth, this.imgHeight);
                const imageData = context.getImageData(0, 0, this.imgWidth, this.imgHeight);
                //obtem o array com a quantidade de pixeis mais proximos de cada cor do histograma
                //este array está na mesma ordem que this.colors
                const hist = h12color.countPixels(imageData);

                // obtem o indice da cor dominante
                let dominantColorIndex = 0;
                let maxCount = hist[0];
                for (let j = 1; j < hist.length; j++) {
                    if (hist[j] > maxCount) {
                        maxCount = hist[j];
                        dominantColorIndex = j;
                    }
                }
                //obtem o nome da cor dominante
                const dominantColor = this.colors[dominantColorIndex];

                colorDatabase.images.push({ class: dominantColor, path: image.path });

                // Atualiza a barra de carregamento
                // calcula o progresso em percentagem
                const progress = (colorDatabase.images.length / this.jsonData.images.length) * 100;
                document.getElementById('loading-bar').style.width = `${progress}%`;

                // quando está completo, salva no local storage
                if (colorDatabase.images.length === this.jsonData.images.length) {
                    this.lsDb.save('colorDatabase', colorDatabase);

                    // esconde a mensagem de carregamento e a barra de carregamento
    ...
    searchColor(color) {
        ...
        //TODO: Implement this method
        //retirei o parametro category
        const colorDatabase = this.lsDb.read('colorDatabase');

        let currentImages = colorDatabase.images
            .filter((image)  => image.class === color) //filtra as imagens que têm a cor passada como parametro
            .map((image) => image.path); //retorna array com os paths

        currentImages = currentImages.slice(0, this.numShownPic)//limita o tamanho do array
        //guarda no local storage
        this.lsDb.save('currentImages', currentImages);
    ...
    searchKeywords(category) {
        ...
        //TODO: Implement this method
        //utiliza o método search da classe DatabaseJSON para obter as imagens
        const currentImages = this.db.search(category, this.jsonData, this.numShownPic);
    ...
    gridView(canvas) {
        ...
        const context = canvas.getContext('2d');
        const images = this.lsDb.read('currentImages'); // lê o local storage
        const canvasWidth = canvas.width;
        const canvasHeight = canvas.height;
        const numImages = images.length;
        
        //nr de linhas, coluna e tamanho de cada imagem baseado no tamanho do canvas
        const numCols = Math.ceil(Math.sqrt(numImages)); //raiz quadrada porque a grid vai ser quadrada - aredondada para cima para sobrar espaço
        const numRows = Math.ceil(numImages / numCols); //ceil no caso de de uma linha não ficar completa
        const imageWidth = canvasWidth / numCols;
        const imageHeight = canvasHeight / numRows;
    
        context.clearRect(0, 0, canvasWidth, canvasHeight);
    
        for (let i = 0; i < numImages; i++) {
            const col = i % numCols; //quando chega ao fim da linha, volta ao inicio
            const row = Math.floor(i / numCols); //quando i ultrapassa o ncols, sobe uma unidade
            //ex: 5/4 = 1.25 que arredonda para 1, ou seja, a 5a imagem vai para a 2a linha
            const x = col * imageWidth;
            const y = row * imageHeight;

            // cria uma instância de Picture e desenha a imagem no canvas
            const img = new Picture(x, y, imageWidth, imageHeight, images[i], "category");
            img.draw(canvas);
        }      
    } 
}
